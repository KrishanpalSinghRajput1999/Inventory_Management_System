...
root = Tk()
root.title("HACKOVERFLOW INVENTORY SYSTEM")
...
root.resizable(0, 0)
root.config(bg="azure4") #change the color
...
def Database():
    ...
    cursor.execute("SELECT * FROM `admin` WHERE `username` = 'Krishanpal' AND `password` = 'Kp@123'")
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `admin` (username, password) VALUES('Krishanpal', 'Kp@123')")
        conn.commit()

def Exit():
    result = tkMessageBox.askquestion('HACKOVERFLOW INVENTORY SYSTEM', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("INVENTORY MANAGEMENT/ACCOUNT LOGIN")
    width = 800
    height = 400
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
